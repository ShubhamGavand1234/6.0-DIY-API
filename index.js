...
app.get("/filter", (req, res) => {
  ...
  var jokeArray = [];
  // jokes.forEach((onejoke) => {
  //   if (onejoke.jokeType === type) {
  //     jokeArray.push(onejoke);
  //   }
  // });

  jokeArray = jokes.filter((joke) => joke.jokeType === type);
  res.json(jokeArray);
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (index > -1) {
    ...
    res.status(404).json({ error: `joke with ${id} not Fond` });
  }
});
...
app.delete("/all", (req, res) => {
  if (req.query.key === masterKey) {
    ...
    res.status(404).json({ error: `key not matching` });
  }
});
